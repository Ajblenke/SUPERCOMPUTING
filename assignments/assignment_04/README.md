...
One of the biggest challenges I ran into during this assignment was accidentally erasing my `~/.bashrc` file. While trying to add my `gh` binary location to `$PATH`, I used `>` instead of `>>` which overwrote the entire file instead of appending to it. This broke my shell environment and I had to figure out how to restore it. I found that `/etc/bashrc` contained the system-wide default configuration for the HPC and was able to copy it back with `cp /etc/bashrc ~/.bashrc` and then re-add my PATH exports. It was a stressful moment but taught me the importance of always using `>>` when appending to important files and the warning in Task 6 about never using `>` with `~/.bashrc` made a lot more sense after going through that experience firsthand.
